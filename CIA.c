...
void get_listNodeToUpdate(int_t i, varNode *Node, int_t N, int_t **Graph, int L, int_t *queue, int_t *check, int_t *lenght) {
...
//return the list with influencers
//first element is # of influencers 
int_t *get_influencers(varNode *Node, int_t N, int_t **Graph, int L) {
	...
	THRESHOLD = pow( ((dbl_t)CI_ave)/NumLink, 1./(L+1.) );
	CUTOFF = 1.-1./(L+1.);
	cnt = 0;
	while( (THRESHOLD > CUTOFF) && (cnt < N)) {
...
int main(int argc, char *argv[]){
	...
	network = argv[1];
	L = atoi(argv[2]);
	sprintf(fname_infl,"INFLUENCERS_<%s>.txt",network);
	...
	fprintf(stdout, "\t\t\t\t  ### DONE ###\n\n");	
	fprintf(stdout, "\t### TO SEE OUTPUT OPEN --> %s ###\n\n", fname_infl);
	fflush(stdout);
	...
	fprintf(list_inf, "# Nodes %ld\n", N); 
	fprintf(list_inf, "#L = %d\n\n", L);
